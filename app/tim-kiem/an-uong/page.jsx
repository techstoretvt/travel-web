...
import { Select, Button, Pagination, Breadcrumb } from 'antd';
import Link from 'next/link';
import { useRouter, useSearchParams } from 'next/navigation'



import styles from './anUong.module.scss'
...
function TimDiaDiem({ params }) {
    ...
    const [loaiQuanAn, setLoaiQuanAn] = useState('all')
    ...
    useEffect(() => {
        if (!loading)
            handleGetAnUong()
    }, [loaiQuanAn, khuVuc, oGan, currentPage])

    const initFetchApi = async () => {
        await handleGetAnUong()
        await handleGetKhuVuc()
        await handleGetOGan()
        setLoading(false)
    }

    const handleGetAnUong = async () => {
        try {
            const city = searchParams.get('city')

            let res = await fetch(process.env.BACKEND + '/api/get-an-uong?type=' + loaiQuanAn + '&khu_vuc=' + khuVuc + '&o_gan=' + oGan + '&city=' + city + '&page=' + currentPage)
        ...
        <div className={styles.DiemDen}>
            ...
            <div className={styles.filters}>
                <div className={styles.filter}>
                    <div className={styles.title}>Loại ăn uống</div>
                    <Select
                        value={loaiQuanAn}
                        style={{
                            minWidth: 120,
                        }}
                        onChange={(value) => onchangeFilter(value, setLoaiQuanAn)}
                        options={[
                            {
                                label: 'Tất cả',
                                value: 'all',
